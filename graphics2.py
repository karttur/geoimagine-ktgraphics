...
class Polyline(GraphicsObject):
    ...
    def __init__(self, *points):
        ...
        if len(points) == 1 and type(points[0]) == type([]):
            points = points[0]
        self.points = list(map(Point.clone, points))
        GraphicsObject.__init__(self, ["width"])
...
def imagedraw(filename):
    win = GraphWin("density", 500, 500)
    anchorpt = Point(250,250)
    c = Image(anchorpt, filename)
    c.draw(win)
...
def DrawImage(win,anchorpt, FPN):
    anchorpt = Point(anchorpt[0],anchorpt[1])
    c = Image(anchorpt, FPN)
    c.draw(win)
    
def DrawPolyline(win,plotL,color):
    if len(plotL) == 0: 
        return
    if len(plotL) == 1:
        c = Point(plotL[0][0],plotL[0][1])
    else:
        ptL = list()
        for p in plotL:
            pt = Point(p[0],p[1])
...
def DrawLine(win,lineL):
    for line in lineL:
        pt1 = Point(line[0][0],line[0][1])
        pt2 = Point(line[1][0],line[1][1])
        l = Line( pt1, pt2)
        l.setWidth(line[2])
        l.setFill(line[3])
        l.draw(win)

def DrawPoint(win,plotL):
    for pt in plotL:
        c = Circle(Point(pt[0],pt[1]), pt[2])
        c.setFill(pt[3])
        c.setOutline(pt[4])
        c.setWidth(2)
        c.draw(win)
        
def DrawCircle(win,pt,radius,color):
    c = Circle(Point(pt[0],pt[1]), radius)
    c.setOutline(color)
    c.draw(win)
                
def DrawText(win,plotL,offset, fs=20, fill = False, textcolor = False, face = False):
    for pt in plotL:
        if pt[2]:
            t = Text(Point(pt[0]+(offset*pt[6]),pt[1]+(offset*pt[7])), pt[5])
            #t.setTextColor('white')
            t.draw(win)
        else:
            t = Text(Point(pt[0],pt[1]), pt[5])
            t.draw(win)
        t.setSize(fs)
        if fill:
            t.setTextColor(pt[4])
...
def CreateCanvas(dim,label):
    win = GraphWin(label, 500, 500)
    win.setCoords(dim[0], dim[2], dim[1], dim[3])
    return win

def ShowWin(win,FPN,show):
    win.postscript(file = FPN)
    if show:
...
if __name__ == "__main__":
    ...
    ##anchorPoint = 'fix'
    #filename = '/Users/thomasg/Documents/mj_ortho/ALSHA_1/blender/IND-lindyn_TC1TC2.gif'
    #imagedraw(filename)
    #vL = [[50,50],[70,50],[90,90]]
    #ptL = coords2pts(vL)
    #Polygondraw(ptL)
